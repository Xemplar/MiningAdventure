...
import static com.xemplar.games.android.miningadv.MiningAdventure.gen;
import static com.xemplar.games.android.miningadv.MiningAdventure.params;
...
public class CompletedLevel implements Screen, InputProcessor {
    ...
    public void render(float delta) {
        ...
        MiningAdventure.layout.setText(text, message);
        float font = MiningAdventure.layout.width;
    ...
    public void resize(int width, int height) {
        ...
        replay = new Button(MiningAdventure.label, "Replay", (width / 2F) - (buttonWidth / 2F), buttonHeight + spacer, buttonWidth, buttonHeight);
        menu = new Button(MiningAdventure.label, "Menu", (width / 2F) - (buttonWidth / 2F), replay.y + spacer + buttonHeight, buttonWidth, buttonHeight);
    }

    public void show() {
        Object det = InterScreenData.getInstance(MiningAdventure.COMP_DATA).getData();
    ...
    public boolean touchUp(int pX, int pY, int pointer, int button) {
        ...
        if(replay.isInside(x, y) && set){
            ...
            MiningAdventure.mining.setScreen(new GameScreen(level));
            value |= true;
        }
